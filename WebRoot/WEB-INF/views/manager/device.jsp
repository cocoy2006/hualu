...
<html>
	<head>
		...
		function openSelectNurseModal(deviceid) {
			var url = host + "selectNurse/";
			$("#modalContainer").load(url, function() {
				$(".modal").modal("show");
				$("tr.nurse").on("click", function() {
					// change nurse
					var nurseid = this.title;
					updateNurse(deviceid, nurseid);
				});
			});
		}
		function updateNurse(deviceid, nurseid) {
			if(deviceid && nurseid) {
				$.ajax({
					url: host + "updateNurse/" + deviceid + "/" + nurseid + "/",
					dataType: "json",
	...
	<body>
		<div class="panel panel-primary">
			<div class="panel-heading">
				...
				<small>点击"领用的客服"<!-- , "绑定的用户" -->姓名可更换</small>
			</div>
